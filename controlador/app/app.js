    ...
    app.post('/Registro-Manzanas', async (req,res)=>{
        const {Nombre_M, Direccion_M} = req.body;
            ...
            if(Indicador.length > 0){
                ...
                window.onload=function(){alert("Manzana ya esta registrada");window.location.href="/Admin"}</script>`);
            }
            else{
                ...
                res.status(201).send('<script>window.onload=function(){alert("Dato registrado");window.location.href="/Admin"}</script>');
            }
            await input.end();
        }
        catch(error){
...
//OBTENER MANZANAS REGISTARDAS
 ...
 app.post('/Actualizar-Manzanas', async (req, res)=>{
    ...
    try{
        ...
        res.status(200).send(`<script> 
        window.onload=function(){alert("Servicio actualizado");window.location.href="/Admin"}
                </script>`)
    }catch (error) { 
        //Capturamos el error.
        console.error('Error en el servidor', error)
 ...
 app.post('/Registro-Servicios', async (req,res)=>{
        ...
        const {nombre_server,tipo_server} = req.body;
            ...
            if(Indica.length > 0){
                 // No existe una manzana con el nombre y la dirección especificados
            res.status(401).send(`<script>  
            window.onload=function(){alert("Servicio ya esta registrado");window.location.href="/Admin"}
            </script>`);
            }
            else{
                 // Ya existe una manzana con el nombre y la dirección especificados
                await input.execute('INSERT INTO servicios(nombre_servicios,tipo) VALUES(?,?)',[nombre_server,tipo_server]);
                res.status(200).send('<script>window.onload=function(){alert("Servicios Regitrado"); window.location.href="/Admin"}</script>');
            }
            await input.end();
        }
        catch(error){
...
//=================================Usuarios Registrados==============================
 app.post('/Registro-Usuarios', async (req,res)=>{
    const input=await mysql.createConnection(db);
    const {nombre_user,tipo_doc,documento,id_usuer} = req.body;
        try{
            const Indicar = await input.execute('SELECT * FROM usuarios WHERE usuarios.nombre_usuarios = ? AND usuarios.tipo_documento = ? AND usuarios.documento = ? AND id_usuarios',[nombre_user,tipo_doc,documento,id_usuer]);
        if(Indicar.length === 0){
            res.status(401).send('<script>window.onload=function(){alert("Usuario ya Registrado");}</script>');
        }
        else{
 ...
 app.delete('/eliminar-user', async (req, res)=>{
    ...
    try{
        ...
        await input.execute('DELETE FROM usuarios WHERE documento = ?',[documento]);
        console.log("Usuario a sido eliminado")
        res.status(200).send(`<script> 
        alert('Servicio eliminado');
        </script>
        `)
